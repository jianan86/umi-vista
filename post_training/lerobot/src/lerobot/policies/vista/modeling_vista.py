...
from lerobot.utils.constants import (
    ...
    OBS_LANGUAGE_TOKENS,
    VISTA_ATTENTION_MASK_VALUE,
)

# from transformers import Qwen3VLForConditionalGeneration, AutoProcessor

# from texas.models.modeling_texas_zdr import TEXAS
# from texas.models.processing_texas import TEXASProcessor
# from texas.models.processing_action_tokenizer import UniversalActionProcessor
...
def create_sinusoidal_pos_embedding(  # reference implementation helper
...
def sample_beta(alpha, beta, bsize, device):  # reference implementation helper
...
def make_att_2d_masks(pad_masks, att_masks):  # reference implementation helper
...
def resize_with_pad_torch(  # reference implementation helper
    images: torch.Tensor,
    height: int,
    width: int,
    mode: str = "bilinear",
...
class GemmaConfig:  # Gemma model config
...
def get_gemma_config(variant: str) -> GemmaConfig:  # Gemma model config factory
...
class PaliGemmaWithExpertModel(
    nn.Module
):  # PaliGemma expert backbone used by VISTA
    """PaliGemma model with action expert for VISTA."""
...
class VISTAPytorch(nn.Module):  # VISTA flow-matching backbone
    """Core VISTA PyTorch model."""

    def __init__(self, config: VISTAConfig):
        super().__init__()
        self.config = config
    ...
    def gradient_checkpointing_enable(self):
        ...
        logging.info("Enabled gradient checkpointing for VISTAPytorch model")
    ...
    def gradient_checkpointing_disable(self):
        ...
        logging.info("Disabled gradient checkpointing for VISTAPytorch model")
    ...
    def _prepare_attention_masks_4d(self, att_2d_masks):
        ...
        return torch.where(att_2d_masks_4d, 0.0, VISTA_ATTENTION_MASK_VALUE)
    ...
    @torch.no_grad()  # VISTA action sampling
...
class VISTAPolicy(PreTrainedPolicy):
    """VISTA Policy for LeRobot."""
    ...
    def __init__(
        ...
        # Initialize the core VISTA model
        self.model = VISTAPytorch(config)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        print(
            "The VISTA policy keeps its checkpoint structure for compatibility. \n"
            "Converted checkpoints should use type=\"vista\"."
        )
        ...
        try:
            ...
            # First, fix any key differences before loading checkpoint weights
    ...
    def _fix_pytorch_state_dict_keys(
    ...
    ):  # VISTA checkpoint key compatibility
        ...
        for key, value in state_dict.items():
            ...
            # Handle MLP naming changes for vista
            # vista model expects time_mlp_*, but checkpoint might have action_time_mlp_*
            if key.startswith("action_time_mlp_in."):
                new_key = key.replace("action_time_mlp_in.", "time_mlp_in.")
            elif key.startswith("action_time_mlp_out."):
                new_key = key.replace("action_time_mlp_out.", "time_mlp_out.")
            # Also handle state_proj which shouldn't exist in vista
            if key.startswith("state_proj."):
                logging.warning(f"Skipping state_proj key in vista mode: {key}")
                continue
    ...
    def _preprocess_images(self, batch: dict[str, Tensor]) -> tuple[list[Tensor], list[Tensor]]:
        ...
        # import ipdb;ipdb.set_trace()
        # LIBERO-UMI compatibility path.
        if len(present_img_keys) == 1 and len(missing_img_keys) == 0:
            missing_img_keys = ['observation.images.head_image']
        # import ipdb;ipdb.set_trace()
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Sample actions using the model (no separate state needed for VISTA)
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # Compute loss (no separate state needed for VISTA)
