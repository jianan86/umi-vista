...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    Args:
        name: The name of the policy. Supported names are "tdmpc", "diffusion", "act",
              "vqbet", "pi0", "pi05", "vista", "sac", "reward_classifier", "smolvla".
    ...
    if name == "tdmpc":
        ...
        from lerobot.policies.vista.modeling_vista import VISTAPolicy
...
def make_policy_config(policy_type: str, **kwargs) -> PreTrainedConfig:
    ...
    Args:
        policy_type: The type of the policy. Supported types include "tdmpc",
                     "diffusion", "act", "vqbet", "pi0", "pi05", "vista", "sac", "smolvla",
                     "reward_classifier".
    ...
    elif policy_type == "vista":
        return VISTAConfig(**kwargs)
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    elif isinstance(policy_cfg, VISTAConfig):
        from lerobot.policies.vista.processor_vista import make_vista_pre_post_processors

        processors = make_vista_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
...
def make_policy(
    ...
    enforce_input_output_replace: bool = True,
) -> PreTrainedPolicy:
    """
    ...
    if not cfg.output_features or enforce_input_output_replace:
        cfg.output_features = {key: ft for key, ft in features.items() if ft.type is FeatureType.ACTION}
    if not cfg.input_features or enforce_input_output_replace:
