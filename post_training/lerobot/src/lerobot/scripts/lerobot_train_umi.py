...
def train(cfg: UmiTrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    cfg.save_as_safetensors = cfg.save_as_safetensors and accelerator.distributed_type != DistributedType.DEEPSPEED
    ...
    # TODO (chenyou): add tensorboard logger
    ...
    ### tb
    writer = SummaryWriter(f"{cfg.output_dir}/tb_log")
    ...
    # TODO: add load_from_safetensors support later
    policy = make_policy(
        cfg=cfg.policy,
        ds_meta=dataset.meta,
        rename_map=cfg.rename_map,
        enforce_input_output_replace=cfg.enforce_input_output_replace,
    )

    ## NOTE: If checkpoint handling is incorrect, add custom save/load hooks following the internal implementation.
    ...
    # import ipdb;ipdb.set_trace()
    # TODO: Review whether delta actions need to be stored inside the dataset.
    ...
    for _ in range(step, cfg.steps):
        ...
        if is_log_step:
            ...
            writer.add_scalar('train/loss', output_dict['loss'], step)
            writer.add_scalar('train/lr', train_tracker.lr.val, step)
        ...
        if cfg.save_checkpoint and is_saving_step:
            ...
            if is_main_process:
                ...
                print(f"good!!!!!!!!!")
                print(f"the ckpt has been saved at {checkpoint_dir}")
