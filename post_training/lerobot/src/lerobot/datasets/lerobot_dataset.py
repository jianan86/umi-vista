...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def load_abs_action_norm_stats(self):
        norm_stats_path = self.root / 'meta' / 'abs_action_norm_stats.json'
        if not norm_stats_path.exists():
            compute_norm_stats(self, keys=self.get_state_action_keys(), use_delta_action=False)
        norm_stats = load_norm_stats(self.root, use_delta_action=False)
        self.meta.update_norm_stats(norm_stats)
    ...
    #     }
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        """Query non-video delta features without decoding unrelated columns.

        `datasets.Dataset.__getitem__` with row indices returns complete rows.  For
        image-in-parquet datasets, using `self.hf_dataset[q_idx][key]` to fetch an
        action/state chunk also decodes every image column in those rows.  Prefer a
        column-only view so delta queries read only the requested feature; keep the
        original row-first path as a compatibility fallback.
        """
        cache = getattr(self, "_hf_dataset_column_cache", None)
        cache_source_id = getattr(self, "_hf_dataset_column_cache_source_id", None)
        if cache is None or cache_source_id != id(self.hf_dataset):
            cache = {}
            self._hf_dataset_column_cache = cache
            self._hf_dataset_column_cache_source_id = id(self.hf_dataset)
        ...
        for key, q_idx in query_indices.items():
            ...
            try:
                hf_dataset_for_key = cache.get(key)
                if hf_dataset_for_key is None:
                    hf_dataset_for_key = self.hf_dataset.select_columns([key])
                    cache[key] = hf_dataset_for_key
                result[key] = torch.stack(hf_dataset_for_key[q_idx][key])
            except Exception:
                # Preserve the previous behavior for older datasets / datasets versions
                # where column-only querying is not supported.
