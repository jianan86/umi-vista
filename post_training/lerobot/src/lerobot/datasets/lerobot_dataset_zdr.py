...
from lerobot.datasets.utils import (
    ...
    load_norm_stats,
    ## legacy version
    legacy_backward_compatible_episodes_stats,
...
class LegacyLeRobotDatasetMetadata:
    def __init__(
        self,
        repo_id: str,
        ...
        self.revision = revision if revision else LEGACY_CODEBASE_VERSION
    ...
    def load_metadata(self):
        ...
        check_version_compatibility(self.repo_id, self._version, LEGACY_CODEBASE_VERSION)
        self.tasks, self.task_to_task_index = legacy_load_tasks(self.root)
        self.episodes = legacy_load_episodes(self.root)
        if self._version < packaging.version.parse("v2.1"):
            self.stats = load_stats(self.root)
            self.episodes_stats = legacy_backward_compatible_episodes_stats(self.stats, self.episodes)
        else:
            self.stats = load_stats(self.root)
            if self.stats is None:
                self.episodes_stats = legacy_load_episodes_stats(self.root)
...
'''
Compared with LeRobot dataset v2, v3 stores q01 and q99 statistics directly in stats; they do not need to be recomputed manually.
'''
class LeRobotDatasetMetadata:
    ...
    def load_metadata(self):
        self.info = load_info(self.root)            # same as the previous version
        check_version_compatibility(self.repo_id, self._version, CODEBASE_VERSION)
        self.tasks = load_tasks(self.root)          # diff from previous version, here would return a parquet
        self.episodes = load_episodes(self.root)    # diff from previous version
        # import ipdb;ipdb.set_trace()
        self.stats = load_stats(self.root)          # same as the operation in v2.0
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.is_legacy_version = packaging.version.parse(self.revision).major < 3
        ...
        if self.is_legacy_version:
            self.meta = LegacyLeRobotDatasetMetadata(
        ...
        try:
            if force_cache_sync:
                raise FileNotFoundError
            self.hf_dataset = self.load_hf_dataset() # if not self.is_legacy_version else self.fast_load_hf_dataset()
            # self.hf_dataset = self.fast_load_hf_dataset() # if not self.is_legacy_version else self.fast_load_hf_dataset()
        ...
        # Get the action feature names
        ### TODO: Handle action.xxx-style feature names when generating the action mask
        action_dim = self.meta.features['action']['shape'][0]
        ...
        # Create the action mask and initialize it to 1
        self.action_mask = torch.ones(action_dim, dtype=torch.bool)
        # Set gripper-related action dimensions to 0
        if len(action_names) == action_dim:
            for i, name in enumerate(action_names):
                if 'gripper' in name.lower():
                    self.action_mask[i] = False
        else:
            # Use the last dimension as the gripper when feature names are ambiguous
        ...
        self.use_corr_state = os.getenv("use_corr_state", "False").lower() in ("true", "1", "yes", "t")
    ...
    def get_max_length(self):
        try:
            jsonl_path = self.root / "meta/episodes_max_length.jsonl"

            self.max_length_per_task = None
            self.max_length = {}
            ...
            with open(jsonl_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    self.max_length[episode_index] = max_length
        
        except Exception as e:
            cprint(f"[Local Rank {self.rank}] {self.repo_id} failed to calculate episode max length. Try to load task max length instead.", msg_type='warning')

            jsonl_path = self.root / "meta/task_max_length.jsonl"

            self.max_length_per_task = {}
            ...
            with open(jsonl_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    max_length = item["max_length"]
                    self.max_length_per_task[task_index] = max_length
    ...
    def get_state_action_keys(self):
        state_keys = [
            key for key, ft in self.meta.features.items() if key.startswith("observation.state")
        ]
        action_keys = [
            key for key, ft in self.meta.features.items() if key.startswith("action")
        ]
    ...
    ## currently only test with v2.0 and v2.1
    def fast_load_hf_dataset(self) -> datasets.Dataset:
        """hf_dataset contains all the observations, states, actions, rewards, etc."""
        if self.episodes is None:
            # import ipdb;ipdb.set_trace()
            # fpaths = list(set([str(self.root / self.meta.get_data_file_path(ep_idx)) for ep_idx in range(self.meta.total_episodes)]))
            fpaths = [str(self.root / self.meta.get_data_file_path(ep_idx)) for ep_idx in self.meta.episodes]
            hf_dataset = datasets.load_dataset("parquet", data_files=fpaths, split="train")
        else:
            files = list(set([str(self.root / self.meta.get_data_file_path(ep_idx)) for ep_idx in self.episodes]))
            hf_dataset = datasets.load_dataset("parquet", data_files=files, split="train")
    ...
    def _get_query_indices(
        self, idx: int, ep_idx: int, delta_indices: dict = None, task_idx: int = None
    ) -> tuple[dict[str, list[int | bool]]]:
        if self.episode_from_index is not None:
            ep_idx = self.episode_from_index[ep_idx]
        # ep_start = self.episode_from_index["from"][ep_idx]
        # ep_end = self.episode_from_index["to"][ep_idx]
        ep = self.meta.episodes[ep_idx]
        ep_start = ep["dataset_from_index"]
        ep_end = ep["dataset_to_index"]

        if self.max_length is not None:
            max_length = self.max_length[ep_idx]
        elif self.max_length_per_task is not None:
            max_length = self.max_length_per_task[task_idx]
        else:
            raise ValueError("No max length found for the dataset")

        steps_to_go = ep_end - idx

        rewards = torch.full((steps_to_go,), -1.0)
        rewards[-1] = 0.0  # Set the last reward to 0

        # Find the first ACTION delta-index entry and use its length
        chunk_size = len(self.delta_indices[self.select_action_keys[0]])
        termination = steps_to_go <= chunk_size

        # normalized with episode max length
        monte_carlo_returns = 1 ** torch.arange(steps_to_go)
        chunk_returns = (monte_carlo_returns * rewards).clone()[:chunk_size].sum() / max_length
        monte_carlo_returns = (monte_carlo_returns * rewards).sum() / max_length
        ...
        return query_indices, padding, monte_carlo_returns, chunk_returns, termination
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.delta_indices is not None:
            ...
            # import ipdb;ipdb.set_trace()
            query_indices, padding, mc_returns, chunk_returns, termination = self._get_query_indices(idx, ep_idx, delta_indices, task_idx=item["task_index"].item())
            # import ipdb;ipdb.set_trace()
        ...
        ep = self.meta.episodes[ep_idx]
        # ep_start = ep["dataset_from_index"]
        ep_end = ep["dataset_to_index"]-1
        next_idx=min(ep_end,idx+self.delta_indices['action'][-1]+1)
        next_item=self.hf_dataset[next_idx]

        item['observation.images.cam_high']=torch.stack([item['observation.images.cam_high'], next_item['observation.images.cam_high']], dim=0)
        item['observation.images.cam_left_wrist']=torch.stack([item['observation.images.cam_left_wrist'], next_item['observation.images.cam_left_wrist']], dim=0)
        item['observation.images.cam_right_wrist']=torch.stack([item['observation.images.cam_right_wrist'], next_item['observation.images.cam_right_wrist']], dim=0)
        item['observation.state']=torch.stack([item['observation.state'], next_item['observation.state']], dim=0)
        # import ipdb;ipdb.set_trace()
        # if len(self.meta.video_keys) > 0:
        ...
        if self.image_transforms is not None:
            ...
            for cam in image_keys:
                if 'wrist' in cam or 'cam_1' in cam or 'cam_2' in cam:  # Treat camera names containing wrist as wrist-view cameras
        ...
        if self.use_corr_state:
            item['observation.state'][-1] = (item['action'][0, -1] * 40).detach().clone()

        # item['index']=idx
        item["target_returns"] = mc_returns
        item["chunk_returns"] = chunk_returns
