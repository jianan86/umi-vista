...
def make_dataset(cfg: TrainPipelineConfig) -> LeRobotDataset | MultiLeRobotDataset:
    ...
    wrist_transforms = (
        ImageTransforms(cfg.dataset.wrist_transforms) if cfg.dataset.wrist_transforms.enable else None
    )

    if isinstance(cfg.dataset.repo_id, str):
        if cfg.dataset.revision is not None and cfg.dataset.revision.startswith('v2'):
        ...
        if not cfg.dataset.streaming:
            import time
            # --- Start timing before dataset construction ---
            ...
            print(f"LeRobotDataset initialization took: {execution_time:.4f} s")
        else:
