...
def compute_norm_stats(
    ...
    if use_delta_action:
        if getattr(dataset, 'delta_indices', None) is None or 'action' not in dataset.delta_indices:
            raise ValueError(
                "delta_indices['action'] is required when use_delta_action=True. "
...
def load_norm_stats(
    ...
    if use_delta_action:
        if action_chunk_size is None:
            # candidates = sorted((local_dir / 'meta').glob('delta_action_ck*_norm_stats.json'))
            # if len(candidates) == 1:
            ...
            # else:
            raise ValueError(
                "Multiple delta-action norm stats files found. "
...
## serve for legacy version of LeRobot Dataset
import jsonlines
...
def legacy_backward_compatible_episodes_stats(
